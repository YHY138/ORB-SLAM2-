...
public:
     ...
     */
    int SearchByProjection(Frame &F, const std::vector<MapPoint*> &vpMapPoints, const float th=3);//通过投影，在当前帧中寻找局部地图中地图点的匹配点
    ...
    // Used in relocalisation (Tracking)用于重定位的
	//将关键帧的地图点投影到当前帧，并寻找匹配点
    ...
    // Matching for the Map Initialization (only used in the monocular case)初始化跟踪
    int SearchForInitialization(Frame &F1, Frame &F2, std::vector<cv::Point2f> &vbPrevMatched, std::vector<int> &vnMatches12, int windowSize=10);

    // Matching to triangulate new MapPoints. Check Epipolar Constraint.通过三角测量产生新的地图点。并通过极线约束来检验
    int SearchForTriangulation(KeyFrame *pKF1, KeyFrame* pKF2, cv::Mat F12,
                               std::vector<pair<size_t, size_t> > &vMatchedPairs, const bool bOnlyStereo);

    // Search matches between MapPoints seen in KF1 and KF2 transforming by a Sim3 [s12*R12|t12]
    // In the stereo and RGB-D case, s12=1。在双目和RGB-D图像中没有相似变换
	//Sim3是相似变换
    int SearchBySim3(KeyFrame* pKF1, KeyFrame* pKF2, std::vector<MapPoint *> &vpMatches12, const float &s12, const cv::Mat &R12, const cv::Mat &t12, const float th);

    // Project MapPoints into KeyFrame and search for duplicated MapPoints.将地图点投影到关键帧，并搜索相同的地图点（关键帧中）
    int Fuse(KeyFrame* pKF, const vector<MapPoint *> &vpMapPoints, const float th=3.0);

    // Project MapPoints into KeyFrame using a given Sim3 and search for duplicated MapPoints.考虑相似变换
