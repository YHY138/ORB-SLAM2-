...
//将一些参数信息写在一个文件中，通过文件进行参数的设置
MapDrawer::MapDrawer(Map* pMap, const string &strSettingPath):mpMap(pMap)//Path文件路径
{
    cv::FileStorage fSettings(strSettingPath, cv::FileStorage::READ);//读取指定文件内容
	//FileStorage会通过关键字来寻找需要输入的参数（关键字就是参数名称）
