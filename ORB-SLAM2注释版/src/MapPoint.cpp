    ...
    mnFound(1), mbBad(false), mpReplaced(NULL), mpMap(pMap)
{
    ...
    pFrame->mDescriptors.row(idxF).copyTo(mDescriptor);//地图点是通过帧创建的时候，将该帧中的对应描述子赋值给这个地图点
...
void MapPoint::AddObservation(KeyFrame* pKF, size_t idx)
{
    ...
    mObservations[pKF]=idx;//设置索引（该地图点在pKF中的索引）

    if(pKF->mvuRight[idx]>=0)//依据存放右图特征点的容器判断该关键帧是由单目、双目/RGB-D产生的
        nObs+=2; // 双目或者grbd
    else
        nObs++; // 单目
}
...
void MapPoint::EraseObservation(KeyFrame* pKF)
{
    bool bBad=false;
    {
        ...
        if(mObservations.count(pKF))
        {
            ...
            if(pKF->mvuRight[idx]>=0)//依据相机类型，减少相应数量的观测帧数
                nObs-=2;
            else
                nObs--;

            mObservations.erase(pKF);//删除
...
int MapPoint::Observations()//返回有多少图像观测到了这个地图点
{
    unique_lock<mutex> lock(mMutexFeatures);
    return nObs;
}
...
void MapPoint::Replace(MapPoint* pMP)//当形成闭环时，pMP和当前地图点代表的是同一个点，那么就用pMP来替换掉当前地图点
//要把当前地图点的观测帧和其他的一些参数都继承给pMP
//最后将当前地图点从地图中删除
//由于将当前地图点的帧都继承给了pMP，所以这样就能够在建图的时候这些帧就能够形成一个闭环
{
    if(pMP->mnId==this->mnId)//确定不是同一个地图点
        return;
    ...
    for(map<KeyFrame*,size_t>::iterator mit=obs.begin(), mend=obs.end(); mit!=mend; mit++)
    {
        ...
        if(!pMP->IsInKeyFrame(pKF))//看当前地图点的观测帧在不在pMP的观测帧容器中
        {
            pKF->ReplaceMapPointMatch(mit->second, pMP);// 让KeyFrame用pMP替换掉原来的MapPoint（将pMP放在当前这个地图点在关键帧地图点容器中所在的位置）
            pMP->AddObservation(pKF,mit->second);// 让MapPoint替换掉对应的KeyFrame（将该关键帧放到pMP的观测帧容器中，并记录pMP在关键帧中的索引）
        }
        else
        {
            // 产生冲突，即pKF中有两个特征点a,b（这两个特征点的描述子是近似相同的），这两个特征点对应两个MapPoint为this,pMP
            // 然而在fuse的过程中pMP的观测更多，需要替换this，因此保留b与pMP的联系，去掉a与this的联系（即认为a之前指向了错误的地图点）
            pKF->EraseMapPointMatch(mit->second);//将pKF帧的地图点容器原本指向当前地图点的指针指向NULL
        }
    }
    pMP->IncreaseFound(nfound);
    pMP->IncreaseVisible(nvisible);
    pMP->ComputeDistinctiveDescriptors();//计算这个地图点的描述子

    mpMap->EraseMapPoint(this);//在地图中删除要被替代的地图点
}
 ...
 */
void MapPoint::IncreaseFound(int n)//增加找到该地图点的帧数
{
    unique_lock<mutex> lock(mMutexFeatures);
    mnFound+=n;
}

float MapPoint::GetFoundRatio()//找到地图点的比率
{
    unique_lock<mutex> lock(mMutexFeatures);
    return static_cast<float>(mnFound)/mnVisible;//找到该地图点的帧数除以能看到该地图点的帧数
}

/**
 * @brief 计算具有代表的描述子，选择该描述子作为地图点的描述子（来说明这个地图点是啥）
 *
...
void MapPoint::ComputeDistinctiveDescriptors()
{
...

    {
        ...
        if(mbBad)
            return;
        observations=mObservations;//获得观测到该地图点的关键帧和其在帧中的索引
    }
    ...
    {
        KeyFrame* pKF = mit->first;//指向一个关键帧

        if(!pKF->isBad())
            vDescriptors.push_back(pKF->mDescriptors.row(mit->second));//关键帧中描述子的行数与索引是对应的
		//获得该地图点在每一个关键帧上的描述子信息
    }
    ...
    {
        Distances[i][i]=0;
        for(size_t j=i+1;j<N;j++)//可以少计算一些
        {
            int distij = ORBmatcher::DescriptorDistance(vDescriptors[i],vDescriptors[j]);
            Distances[i][j]=distij;//整个容器实际上是一个对称的数组
    ...
    for(size_t i=0;i<N;i++)
    {
        ...
        // 获得中值
        int median = vDists[0.5*(N-1)];//中值指的是排序后的中间值，不是求整个容器的中值
        
        // 寻找最小的中值
        if(median<BestMedian)
        {
            BestMedian = median;
            BestIdx = i;//该最小值所对应的关键帧在obersevation中的位置
        }
    }
...
cv::Mat MapPoint::GetDescriptor()//获得地图点的描述子
{
    unique_lock<mutex> lock(mMutexFeatures);
...
{
    unique_lock<mutex> lock(mMutexFeatures);
    if(mObservations.count(pKF))//首先判断关键帧是否观测到了这个地图点
        return mObservations[pKF];//然后返回map容器中pKF对应的size_t--索引
    else
        return -1;
}
...
{
    unique_lock<mutex> lock(mMutexFeatures);
    return (mObservations.count(pKF));//count查看map容器中是否有pKF这个元素
}
 ...
 * @see III - C2.2 c2.4
 */
void MapPoint::UpdateNormalAndDepth()//这里只在有新的关键帧观测到一个地图点时才进行更新
{
    map<KeyFrame*,size_t> observations;
    KeyFrame* pRefKF;
    cv::Mat Pos;
    {
        ...
        observations=mObservations; // 获得观测到该3d点的所有关键帧，以及该关键点在KF中的索引
    ...
    {
        KeyFrame* pKF = mit->first;
        cv::Mat Owi = pKF->GetCameraCenter();//获得关键帧对应的相机中心
        cv::Mat normali = mWorldPos - Owi;//计算从相机中心指向地图点的向量
    ...
    const int level = pRefKF->mvKeysUn[observations[pRefKF]].octave;//该地图点在关键帧中被观测到的层数
    const float levelScaleFactor =  pRefKF->mvScaleFactors[level];//该层与底层的缩放倍数
...

    {
        ...
        mNormalVector = normal/n;                                        // 获得平均的观测方向？？为什么除以总数就是平均了？这样只是将方向向量缩小了而已吧
    }
}
...
//           /________\   level:m   --> d
//          /__________\                     dmax/d = 1.2^m//两层距离比与两层对应缩放倍数有关
// Farther /____________\ level:0   --> dmax
//
//           log(dmax/d)
// m = ceil(------------)，1.2是两层间的放大倍数？
//            log(1.2)
int MapPoint::PredictScale(const float &currentDist, KeyFrame* pKF)//和下面那个算法似乎一样
{
    float ratio;
    {
        unique_lock<mutex> lock(mMutexPos);
