...
//由于KF是从F中衍生出来的，所以可以用F中的参数来初始化KF
    ...
    mbToBeErased(false), mbBad(false), mHalfBaseline(F.mb/2), mpMap(pMap)
{
    ...
    for(int i=0; i<mnGridCols;i++)//将F中的网格信息都移植到KF的网格中
    {
...
void KeyFrame::SetPose(const cv::Mat &Tcw_)
{
    ...
    cv::Mat center = (cv::Mat_<float>(4,1) << mHalfBaseline, 0 , 0, 1);//双目中心的齐次坐标（在相机坐标系下）
    // 世界坐标系下，左目相机中心到立体相机中心的向量，方向由左目相机指向立体相机中心
    Cw = Twc*center;//双目中点的世界坐标
}

cv::Mat KeyFrame::GetPose()//获得从世界坐标系到相机坐标系的位姿变换
{
    unique_lock<mutex> lock(mMutexPose);
    return Tcw.clone();
}

cv::Mat KeyFrame::GetPoseInverse()//与上述相反
{
    unique_lock<mutex> lock(mMutexPose);
    return Twc.clone();
}

cv::Mat KeyFrame::GetCameraCenter()//获得相机中心或双目的左相机
{
    unique_lock<mutex> lock(mMutexPose);
    return Ow.clone();
}

cv::Mat KeyFrame::GetStereoCenter()//获得双目中点
{
    unique_lock<mutex> lock(mMutexPose);
    return Cw.clone();
}

//一般都是获得从世界坐标系到相机坐标系下的转换矩阵
 ...
 */
void KeyFrame::AddConnection(KeyFrame *pKF, const int &weight)//只是设置一下与该KF相关的pKF的权重而已。与该KF有联系的KFs已经被保存在了mConnectedKeyFrameWeights中
{
    {
        ...
        if(!mConnectedKeyFrameWeights.count(pKF)) // count函数返回0，mConnectedKeyFrameWeights中没有pKF，之前没有连接
            mConnectedKeyFrameWeights[pKF]=weight;//若之前没有连接，则将pKF放进map中，并设置相应权重
        else if(mConnectedKeyFrameWeights[pKF]!=weight) // pKF已被保存，则修改pKF的权重
...
void KeyFrame::UpdateBestCovisibles()
{
    ...
    // 按照权重进行排序（sort就对每个元素所包含的第一个值的大小进行排序）
    sort(vPairs.begin(),vPairs.end());//从小到大的排序
...
vector<KeyFrame*> KeyFrame::GetCovisiblesByWeight(const int &w)
{
    ...
    vector<int>::iterator it = upper_bound(mvOrderedWeights.begin(),mvOrderedWeights.end(),w,KeyFrame::weightComp);//最后参数是一个函数。即比较map
    //中元素和w的值
    if(it==mvOrderedWeights.end() && *mvOrderedWeights.rbegin()<w)//*mvOrderedWeights.rbegin()返回容器中最后一个元素。这里使用的是排序后的权重，故最后一个应为最大的权重
    //
        return vector<KeyFrame*>();
    else
    {
...
void KeyFrame::AddMapPoint(MapPoint *pMP, const size_t &idx)
{
    ...
    mvpMapPoints[idx]=pMP;//地图点索引和特征点索引是一致的
}

void KeyFrame::EraseMapPointMatch(const size_t &idx)//消除某个特征点（idx）与其目前对应的地图点的联系
{
    unique_lock<mutex> lock(mMutexFeatures);
    mvpMapPoints[idx]=static_cast<MapPoint*>(NULL);
}

void KeyFrame::EraseMapPointMatch(MapPoint* pMP)//消除地图点pMP与其目前所对应的特征点的联系
{
 ...
 * @param  minObs 最小观测
 */
int KeyFrame::TrackedMapPoints(const int &minObs)//获得当前KF的地图点中被观测次数高于minObs的地图点的数量
{
    unique_lock<mutex> lock(mMutexFeatures);
...
/*功能：
1、提取出当前KF中的地图点
2、获得每个地图点的所有观测帧，并计算这些观测帧与当前帧的共视点数量
3、挑选共视点数量足够的KFs，建立它们与当前KF的权重边（权重为共视点数量）
注意，权重边是从其他KFs到当前KF*/
void KeyFrame::UpdateConnections()
{
    ...
    for(vector<MapPoint*>::iterator vit=vpMP.begin(), vend=vpMP.end(); vit!=vend; vit++)
    {
        ...
        for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(mit->first->mnId==mnId)
                continue;
            KFcounter[mit->first]++;//增加某个pKF与当前KF的共视3D点数目
        }
    }
    ...
    //===============2==================================
    // If the counter is greater than threshold add connection将大于阈值（共视点数目）的关键帧与当前在建立联系
    // In case no keyframe counter is over threshold add the one with maximum counter如果没有一个大于阈值，那么就把KFcounter中权重最大的那个建立边
    int nmax=0;//记录最大权重数，以及对应的关键帧
    KeyFrame* pKFmax=NULL;
    int th = 15;

    // vPairs记录与其它关键帧共视帧数大于th的关键帧
    // pair<int,KeyFrame*>将关键帧的权重写在前面，关键帧写在后面方便后面排序
    vector<pair<int,KeyFrame*> > vPairs;//记录共视点数量足够的KFs
...
bool KeyFrame::hasChild(KeyFrame *pKF)//检查当前KF是否有pKF这个子节点
{
    unique_lock<mutex> lockCon(mMutexConnections);
    return mspChildrens.count(pKF);
}

void KeyFrame::AddLoopEdge(KeyFrame *pKF)//与当前KF构成回环的帧？？
{
    unique_lock<mutex> lockCon(mMutexConnections);
    mbNotErase = true;
...
void KeyFrame::EraseConnection(KeyFrame* pKF)//删除当前KF与pKF的连接
{
    bool bUpdate = false;
    {
        unique_lock<mutex> lock(mMutexConnections);
    ...
    if(bUpdate)
        UpdateBestCovisibles();//删除之后对所有与当前KF有联系的KFs进行新的排序
}

// r为边长（半径）。在Area中有哪些特征点
vector<size_t> KeyFrame::GetFeaturesInArea(const float &x, const float &y, const float &r) const
{
    ...
    const int nMinCellX = max(0,(int)floor((x-mnMinX-r)*mfGridElementWidthInv));//这个Area左边界在图像的第几个网格中（横向）
    if(nMinCellX>=mnGridCols)
        return vIndices;

    // ceil向上取整
    const int nMaxCellX = min((int)mnGridCols-1,(int)ceil((x-mnMinX+r)*mfGridElementWidthInv));//右边界（横向）
    if(nMaxCellX<0)
        return vIndices;

    const int nMinCellY = max(0,(int)floor((y-mnMinY-r)*mfGridElementHeightInv));//同上，纵向
    ...
    for(int ix = nMinCellX; ix<=nMaxCellX; ix++)
    {
        ...
        {
            const vector<size_t> vCell = mGrid[ix][iy];//在（ix，iy）这个网格中的特征点的索引
            for(size_t j=0, jend=vCell.size(); j<jend; j++)
            {
                ...
                if(fabs(distx)<r && fabs(disty)<r)//判断特征点是否在半径为r的范围内
 ...
 * @return   3D点（相对于世界坐标系）
 */
cv::Mat KeyFrame::UnprojectStereo(int i)//KF中的2D点向3D点转换（转换成世界坐标）
{
 ...
 * @return Median Depth
 */
float KeyFrame::ComputeSceneMedianDepth(const int q)//返回KF中所有地图点深度的中值（所有像素深度中的中间值）
{
    ...
    Rcw2 = Rcw2.t();
    float zcw = Tcw_.at<float>(2,3);//在Z轴上的移动
    for(int i=0; i<N; i++)
    {
        if(mvpMapPoints[i])
        {
