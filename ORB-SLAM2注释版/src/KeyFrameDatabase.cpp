    ...
    mpVoc(&voc)
{
	...
	//将mvInvertedFile为每个word都设置一个list容器，这个容器储存每个word它们出现在的关键帧中
}
...
void KeyFrameDatabase::add(KeyFrame *pKF)
{
    ...
    // 为每一个word添加该KeyFrame
	//给pKF所包含的每个word都添加指向pKF的指针
    for(DBoW2::BowVector::const_iterator vit= pKF->mBowVec.begin(), vend=pKF->mBowVec.end(); vit!=vend; vit++)
        mvInvertedFile[vit->first].push_back(pKF);//vit->first应该是word的编号（在BowVector中的编号顺序=mvInverteFile中的顺序）
}
...
void KeyFrameDatabase::erase(KeyFrame* pKF)
{
    ...
    for(DBoW2::BowVector::const_iterator vit=pKF->mBowVec.begin(), vend=pKF->mBowVec.end(); vit!=vend; vit++)
    {
        ...
        list<KeyFrame*> &lKFs = mvInvertedFile[vit->first];//获得包含pKF中某个word的所有关键帧

        for(list<KeyFrame*>::iterator lit=lKFs.begin(), lend= lKFs.end(); lit!=lend; lit++)//找到要删除的关键帧，删除它
        {
            if(pKF==*lit)
            {
                lKFs.erase(lit);
...
{
    // 提出所有与该pKF相连（和pKF联系不超过十位数帧的）的KeyFrame，这些相连Keyframe都是局部相连，在闭环检测的时候将被剔除
    set<KeyFrame*> spConnectedKeyFrames = pKF->GetConnectedKeyFrames();
    list<KeyFrame*> lKFsSharingWords;// 用于保存可能与pKF形成回环的候选帧（只要有相同的word，且不属于局部相连帧）

    // Search all keyframes that share a word with current keyframes
    // Discard keyframes connected to the query keyframe
    // 步骤1：找出和当前帧具有公共单词的所有关键帧（不包括与当前帧链接的关键帧）==找出除spConnectedKeyFrames中帧以外的，和当前帧共享word的所有关键帧
    {
        unique_lock<mutex> lock(mMutex);

        // words是检测图像是否匹配的枢纽，遍历该pKF的每一个word
        for(DBoW2::BowVector::const_iterator vit=pKF->mBowVec.begin(), vend=pKF->mBowVec.end(); vit != vend; vit++)//遍历pKF所包含的word
        {
            // 提取所有包含该word的KeyFrame
            list<KeyFrame*> &lKFs =   mvInvertedFile[vit->first];

            for(list<KeyFrame*>::iterator lit=lKFs.begin(), lend= lKFs.end(); lit!=lend; lit++)//遍历word所关系到的关键帧
            {
                KeyFrame* pKFi=*lit;
                if(pKFi->mnLoopQuery!=pKF->mnId)// pKFi还没有标记为pKF的候选帧
                {
                    ...
                    {
                        pKFi->mnLoopQuery=pKF->mnId;// pKFi标记为pKF的候选帧，之后直接跳过判断（标记之后可以直接进行word++）
    ...
    list<pair<float,KeyFrame*> > lScoreAndMatch;//记录每个候选帧的得分
    ...
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
        ...
        if(pKFi->mnLoopWords>minCommonWords)
        {
            ...
            float si = mpVoc->score(pKF->mBowVec,pKFi->mBowVec);//计算两个关键帧的词袋间的得分（相似度）

            pKFi->mLoopScore = si;//闭环得分
            if(si>=minScore)//得分大于最小值则保存
	...
	//解析：在上述的代码中已经找出了满足相似性的所有候选帧，并记录了他们的得分。之后再候选者中的一个为主，将与其共视程度最高的10个关键帧一起归为一组，计算它们的累计得分。
	//同时记录出该组的分数最高的关键帧。组中成员KF只有同为当前KF候选帧的情况下才能为组贡献得分
	//或许是认为组得分越高，越代表这些关键帧表示的场景与当前帧相同
    for(list<pair<float,KeyFrame*> >::iterator it=lScoreAndMatch.begin(), itend=lScoreAndMatch.end(); it!=itend; it++)
    {
        KeyFrame* pKFi = it->second;
        vector<KeyFrame*> vpNeighs = pKFi->GetBestCovisibilityKeyFrames(10);//获取共视性最高的十个关键帧
        ...
        {
            KeyFrame* pKF2 = *vit;
            if(pKF2->mnLoopQuery==pKF->mnId && pKF2->mnLoopWords>minCommonWords)//组中成员KF只有同为当前KF候选帧的情况下才能为组贡献得分
            {
    ...
    for(list<pair<float,KeyFrame*> >::iterator it=lAccScoreAndMatch.begin(), itend=lAccScoreAndMatch.end(); it!=itend; it++)
    {
        ...
        {
            KeyFrame* pKFi = it->second;//组中得分最高的KF
    ...
    return vpLoopCandidates;//返回最终的一些候选者
}
...
{
    // 相对于关键帧的闭环检测DetectLoopCandidates，重定位检测中没法获得相连的关键帧（因为需要重定位的帧相当于是突然插进来的）
    ...
    int maxCommonWords=0;
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)//找出候选帧中的最多相似单词/帧
    {
    ...
    list<pair<float,KeyFrame*> > lAccScoreAndMatch;//组得分和组中最高分的帧
