                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    // mvbMatched1记录每个特征点是否有匹配的特征点（图1中的特征点），
    // 这个变量后面没有用到，后面只关心匹配上的特征点
    mvbMatched1.resize(mvKeys1.size());//记录的是参考帧中的特征点是否匹配成功

    // 步骤1：组织特征点对 {i, matched(i)}i是参考帧的特征点索引，matched(i)是匹配的当前帧的特征点的索引
    ...
    //为什么这里直接使用0-N作为索引？这0-N并不一定是那些匹配好的点。还是说这个索引是给mvMatches12使用的？？在八点法时使用
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        for(size_t j=0; j<8; j++)//为每次迭代选择八个特征点
        {
            // 产生0到N-1的随机数
 ...
 */
void Initializer::FindHomography(vector<bool> &vbMatchesInliers, float &score, cv::Mat &H21)//每次迭代在计算单应矩阵时作为内点的vbMatchesInliers中的匹配点对
{
    ...
    // Iteration variables
    vector<cv::Point2f> vPn1i(8);//八点法计算时图1的索引
    vector<cv::Point2f> vPn2i(8);//图2的索引
    cv::Mat H21i, H12i;
    ...
    for(int it=0; it<mMaxIterations; it++)//迭代多次，选择得分最高的那次（因为每次迭代的索引都是用随机数产生的）
    {
        // Select a minimum set
        for(size_t j=0; j<8; j++)
        {
            int idx = mvSets[it][j];//每次迭代所使用的八个特征点的索引
        ...
        cv::Mat Hn = ComputeH21(vPn1i,vPn2i);//八点法计算H
        // 恢复原始的均值和尺度？？？？？？
        H21i = T2inv*Hn*T1;
        H12i = H21i.inv();

        // 利用重投影误差为当次RANSAC的结果评分
        currentScore = CheckHomography(H21i, H12i, vbCurrentInliers, mSigma);//判断在当前H下，匹配点中好点的个数
 ...
 */
//给给定的单应矩阵打分，并判断那些匹配点是当前单应矩阵所对应的内点
...
{   
    ...
    for(int i=0; i<N; i++)
    {
        ...
        const float u2in1 = (h11inv*u2+h12inv*v2+h13inv)*w2in1inv;//要将第三个坐标的值化为1
 ...
 */
//通过判断匹配点到对应极线的距离来打分
float Initializer::CheckFundamental(const cv::Mat &F21, vector<bool> &vbMatchesInliers, float sigma)//返回的是作为该基础矩阵内点的匹配点对中各自到相应极线的距离总和
{
    const int N = mvMatches12.size();
 ...
 */
/*步骤：1、先通过基础矩阵和内参矩阵求出本质矩阵E
2、在通过对E进行分解，求出R、t的四种组合；
3、通过当前F的内点和它们的深度情况来记录上述每种组合的情况
4、通过上述组合产生的好点个数以及它们好点中较大视差角来决定哪一种组合是合适的
*/
                            ...
                            cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated)
{
    ...
    int nMinGood = max(static_cast<int>(0.9*N),minTriangulated);//最少应该有多少个可以构造出3D点的好点
    ...
    if(maxGood==nGood1)
    {
        if(parallax1>minParallax)//视差角要大于最小要求
        {
            vP3D = vP3D1;
                      ...
                      cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated)
{
    int N=0;
    for(size_t i=0, iend = vbMatchesInliers.size() ; i<iend; i++)
        if(vbMatchesInliers[i])//统计是当前H矩阵的内点的点个数
            N++;
    ...
    cv::Mat invK = K.inv();
    cv::Mat A = invK*H21*K;//A似乎是相机坐标系间的换算
    ...
    1、最优解的好点个数要比次优解的多很多；
    2、最优解的较大视差角要满足要求；
    3、最优解好点个数要大于最少可三角化点的个数；
    4、最优解好点个数要占当前单应矩阵内点数量的90%*/
...
void Initializer::Triangulate(const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, const cv::Mat &P1, const cv::Mat &P2, cv::Mat &x3D)
{
    ...
    x3D = vt.row(3).t();
    x3D = x3D.rowRange(0,3)/x3D.at<float>(3);//三角化以后，将3D点坐标变成齐次坐标
}
...
void Initializer::Normalize(const vector<cv::KeyPoint> &vKeys, vector<cv::Point2f> &vNormalizedPoints, cv::Mat &T)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        vNormalizedPoints[i].y = vKeys[i].pt.y - meanY;//以此获得的vNormalizedPoints中点的坐标的均值为0

        meanDevX += fabs(vNormalizedPoints[i].x);
        meanDevY += fabs(vNormalizedPoints[i].y);
    }

    meanDevX = meanDevX/N;//求得vNormalizedPoints中点的坐标的一阶绝对矩
    meanDevY = meanDevY/N;
    ...
    for(int i=0; i<N; i++)
    {
        vNormalizedPoints[i].x = vNormalizedPoints[i].x * sX;//这就使得vNormalizedPoints中点的坐标的一阶绝对矩为1
 ...
 */
//获得某一组R、t的好点个数（通过判断经过当前R、t变换后，3D点的深度是否为正，以及视差角是否不是很大）以及好点中的较大的视差角
                       ...
                       const cv::Mat &K, vector<cv::Point3f> &vP3D, float th2, vector<bool> &vbGood, float &parallax)
{
    ...
    vP3D.resize(vKeys1.size());//保存图像1每个特征点所对应的3D点
    ...
    // 步骤1：得到一个相机的投影矩阵
    // 以第一个相机的光心作为世界坐标系
    cv::Mat P1(3,4,CV_32F,cv::Scalar(0));//P1是世界坐标系的点投影到图像1的投影矩阵（这里将第一个相机的坐标系作为世界坐标系）
    ...
    for(size_t i=0, iend=vMatches12.size();i<iend;i++)
    {
        if(!vbMatchesInliers[i])//判断是否是内点，不是内点的话就不能用它的深度情况判断变换矩阵
            continue;
        ...
        // 步骤3：利用三角法恢复三维点p3dC1
        Triangulate(kp1,kp2,P1,P2,p3dC1);//恢复的3D点是在第一个相机面前的
        ...
        float cosParallax = normal1.dot(normal2)/(dist1*dist2);//视差余弦值：3D点到两个相机光心的连线间的夹角

        // 步骤5：判断3D点是否在两个摄像头前方

        // Check depth in front of first camera (only if enough parallax, as "infinite" points can easily go to negative depth)
        // 步骤5.1：3D点深度为负，在第一个摄像头后方，淘汰
        if(p3dC1.at<float>(2)<=0 && cosParallax<0.99998)//？？深度为负仅对对那些视差较大（距离较近）的点有效，视差较小（距离远）的点
            //很容易被检测成负深度
            continue;

        // Check depth in front of second camera (only if enough parallax, as "infinite" points can easily go to negative depth)
        // 步骤5.2：3D点深度为负，在第二个摄像头后方，淘汰
        cv::Mat p3dC2 = R*p3dC1+t;//转换到第二个相机坐标系下
    ...
    if(nGood>0)
    {
        ...
        parallax = acos(vCosParallax[idx])*180/CV_PI;//arccos转换回来的是以派为单位的角度值
    }
    else
        parallax=0;
